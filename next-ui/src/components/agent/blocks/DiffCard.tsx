...
type KeyedDiffLine = { readonly key: string; readonly line: DiffBlock["lines"][number] };

function keyedDiffLines(lines: DiffBlock["lines"]): readonly KeyedDiffLine[] {
  const occurrences = new Map<string, number>();
  const keyed: KeyedDiffLine[] = [];
  for (const line of lines) {
    const baseKey = `${line.type}:${line.text}`;
    const occurrence = occurrences.get(baseKey) ?? 0;
    occurrences.set(baseKey, occurrence + 1);
    keyed.push({ key: occurrence === 0 ? baseKey : `${baseKey}#${occurrence + 1}`, line });
  }
  return keyed;
}
    ...
    >
      ...
      <Collapse in={open} unmountOnExit>
        <Box sx={{ borderTop: (t) => `1px solid ${t.custom.borders.subtle}`, backgroundColor: (t) => t.custom.surfaces.s1, py: 0.5 }}>
          {keyedDiffLines(block.lines).map(({ key, line }) => (
            <Box
              key={key}
              sx={{
                display: "flex",
