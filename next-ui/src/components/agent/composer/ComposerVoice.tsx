...
function voiceLevelSlots(levels: readonly number[]): readonly { readonly key: string; readonly level: number; readonly slot: number }[] {
  const slots: { readonly key: string; readonly level: number; readonly slot: number }[] = [];
  let slot = 0;
  for (const level of levels) {
    slots.push({ key: `voice-slot-${slot}`, level, slot });
    slot += 1;
  }
  return slots;
}
