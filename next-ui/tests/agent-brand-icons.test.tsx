...
describe("agent brand icons", () => {
  it.each(["codex", "opencode"] as const)("renders %s as a brand SVG in monogram tiles", (agent) => {
    ...
    expect(queryByText(agent === "codex" ? "CX" : "OC")).not.toBeInTheDocument();
  });

  it.each(["codex", "opencode"] as const)("renders %s as a brand SVG in compact glyphs", (agent) => {
