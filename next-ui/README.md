...
`npm start` (and the `npx` bin) runs `bin/next-ui.mjs`, which:
...
| `NEXT_UI_DEMO`    | _(unset)_    | Set to `1` to seed demo conversations on first run.               |
| `NEXT_UI_DATA_DIR`| `./.data`    | Where persisted workspace state is stored (relative or absolute). |
...
projects. State is persisted to `NEXT_UI_DATA_DIR` (`.data/` by default) the
first time the server runs. To preview the app with sample data, start it with
`NEXT_UI_DEMO=1` (this only affects the *first* seed; an existing state file is
always loaded as-is).

### Running as a service

`bin/next-ui.mjs` is a plain long-running Node process, so it wraps cleanly in
`systemd`, `pm2`, Docker, etc. Point `NEXT_UI_DATA_DIR` at a writable data volume
and set `PORT`/`HOST` as needed. Example `systemd` unit:

```ini
[Service]
ExecStart=/usr/bin/npx @kanban/next-ui
Environment=PORT=4280
Environment=HOST=127.0.0.1
Environment=NEXT_UI_DATA_DIR=/var/lib/next-ui
Restart=on-failure
```
...
`NEXT_UI_DEMO`), so you have something to look at immediately.
...
| `npm start`            | Build-if-needed and serve the prod SPA + API (`bin/next-ui.mjs`). |
...
- `bin/next-ui.mjs` — the production launcher.
