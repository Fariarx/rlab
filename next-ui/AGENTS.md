...
## Gotchas
- **Frequent `Workspace save failed (502)` = the event loop is blocked by big-blob persistence, NOT a proxy fault.** The whole `workspace-state.json` (multiple MB on long threads) is rewritten on every persist, and a streaming agent run triggers a persist per (throttled) tick — each one a sync `JSON.stringify` of MBs + a `.bak` copy of MBs + a full re-read. With a few-MB state and/or several concurrent runs the sync writes pin the event loop; every UI `PUT /api/workspace` then queues and times out → Caddy returns **502** (and `GET /api/health` can take 20s while `storage.ok` is still `true`). Fix in `vite-agents-plugin.ts` background-run persistence: scale the persist interval with state size, **skip the `.bak` on the hot path** (the temp+rename is already atomic), and **coalesce writes across runs** (`writeJsonFileAtomic(...,backup=false)`, `lastWorkspaceStateBytes`, `lastBackgroundPersistAt`, `scheduleBackgroundRunPersist`). To confirm the diagnosis live: `ps -o %cpu` on the rlab node (spinning) and the listen-socket backlog `ss -ltn 'sport = :4280'` (a non-zero `Recv-Q` = the loop isn't accepting). A long stuck agent run + a multi-MB blob is the classic trigger; killing the run and restarting clears it, but the persistence has to be cheap or it recurs.
