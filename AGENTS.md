...
- On the shared rlab host, prod/devprod agent runtime may be launched from `/root/workspace/rlab/next-ui/bin/rlab.mjs`, and Claude MCP config points directly at `/root/workspace/rlab/next-ui/bin/rlab-mcp-stdio.mjs`. New agent runs read that working-tree MCP script even without a frontend deploy, so dirty edits to `next-ui/bin/rlab-mcp-stdio.mjs` can change available MCP tool names on prod/devprod. Check `ps auxww` and Claude MCP logs before assuming an undeployed tool rename cannot be live.
